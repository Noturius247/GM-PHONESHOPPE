...
class _SkyPageState extends State<SkyPage> with TickerProviderStateMixin {
  late AnimationController _scaleAnimationController;
  late Animation<double> _scaleAnimation;

  // Sky gradient colors matching admin page
  ...
  Future<void> _loadCustomers() async {
    ...
    // Load sort setting
    final sortSetting = await POSSettingsService.getSortSetting('sky');

    var customers = await FirebaseDatabaseService.getCustomers(FirebaseDatabaseService.sky);
    ...
    final result = await Navigator.push<MultiScanResult>(
      ...
      MaterialPageRoute(
        builder: (context) => MultiScannerPage(
          serviceType: FirebaseDatabaseService.sky,
          serviceName: 'Sky',
          primaryColor: skyGradient[0],
        ),
      ),
    );
  ...
  Future<void> _processMultiScanResult(MultiScanResult? result) async {
    ...
    // Show add customer dialog with scanned code as Box Number
    // Box Number duplicates are allowed
    if (!mounted) return;
    _showAddCustomerDialogWithMultiScan(result);
  }

  void _showAddCustomerDialogWithMultiScan(MultiScanResult scanResult) {
    // For Sky, use first scanned code as Box Number (primary)
    final scannedBoxNumber = scanResult.allScannedItems.isNotEmpty
        ? scanResult.allScannedItems.first.code
        : '';
    final boxNumberController = TextEditingController(text: scannedBoxNumber);
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  // Box Number field (primary identifier for Sky) - MultiScan dialog
                  _buildTextField(
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: Icon(Icons.store, color: skyGradient[0]),
                      filled: true,
                      ...
                      focusedBorder: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(12),
                        borderSide: BorderSide(color: skyGradient[0]),
                      ),
                    ),
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    decoration: InputDecoration(
                      labelText: 'Status',
                      labelStyle: TextStyle(color: Colors.white.withValues(alpha: 0.7)),
                      prefixIcon: Icon(Icons.flag, color: skyGradient[0]),
                      filled: true,
                      fillColor: Colors.white.withValues(alpha: 0.05),
                      enabledBorder: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(12),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final boxIdExistsCheck = await FirebaseDatabaseService.boxIdExists(
                  FirebaseDatabaseService.sky,
                  boxIdController.text,
                );
                if (boxIdExistsCheck) {
                  if (!context.mounted) return;
                  final existingCustomer = await FirebaseDatabaseService.getCustomerByBoxId(
                    FirebaseDatabaseService.sky,
                    boxIdController.text,
                  );
  ...
  Future<void> _processOcrData(OcrExtractedData? ocrData) async {
    ...
    // Box Number duplicates are allowed, proceed to add customer dialog
    if (!mounted) return;
    _showAddCustomerDialogWithOcrData(ocrData);
  }

  void _showAddCustomerDialogWithOcrData(OcrExtractedData ocrData) {
    // For Sky, use any detected number as Box Number (primary - prefer boxNumber, then serial, then cca)
    final detectedBoxNumber = ocrData.boxNumber ?? ocrData.serialNumber ?? ocrData.ccaNumber ?? '';
    final boxNumberController = TextEditingController(text: detectedBoxNumber);
    // Serial Number or CCA = Box ID for Sky
    final boxIdController = TextEditingController(text: ocrData.serialNumber ?? ocrData.ccaNumber ?? '');
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          title: Row(
            children: [
              Icon(Icons.document_scanner, color: skyGradient[0], size: 24),
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  // Box Number field (primary identifier for Sky) - OCR dialog
                  _buildTextField(
                    controller: boxNumberController,
                    label: 'Box Number *',
                    icon: Icons.inventory_2,
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Choose how to enter the Box ID',
                  style: TextStyle(color: Colors.white.withValues(alpha: 0.7)),
                ),
                ...
                _buildScanOptionTile(
                  ...
                  title: 'Scan Barcodes',
                  subtitle: 'Scan Box ID and other codes',
                  onTap: () {
                ...
                _buildScanOptionTile(
                  ...
                  subtitle: 'Take photo of Box ID text',
                  onTap: () {
                    Navigator.pop(context);
                    _openOcrScanner();
                  },
  ...
  Widget _buildScanOptionTile({
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: skyGradient[0].withValues(alpha: 0.1),
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: skyGradient[0].withValues(alpha: 0.3)),
        ),
        child: Row(
          children: [
            Container(
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          title: Row(
            children: [
              Icon(Icons.edit, color: skyGradient[0], size: 24),
              const SizedBox(width: 8),
              const Text(
                'Manual Entry',
                style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold),
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildConfirmRow('Supplier', supplier),
  ...
  Widget _buildTextField({
    ...
    return TextField(
      ...
      style: const TextStyle(color: Colors.white),
      cursorColor: skyGradient[0],
      keyboardType: keyboardType,
      maxLines: maxLines,
      decoration: InputDecoration(
        labelText: label,
        labelStyle: TextStyle(color: Colors.white.withValues(alpha: 0.7)),
        prefixIcon: Icon(icon, color: skyGradient[0]),
        filled: true,
        fillColor: Colors.white.withValues(alpha: 0.05),
        enabledBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(12),
  ...
  Widget _buildDateField({
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Icon(Icons.calendar_today, color: skyGradient[0]),
  ...
  void _showCustomerDetailsDialog(Map<String, dynamic> customer) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            Icon(Icons.satellite_alt, color: skyGradient[0], size: 24),
            const SizedBox(width: 8),
            const Expanded(
              child: Text(
                'Customer Details',
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              if (customer['boxId'] != null)
                _detailRow('Box ID', customer['boxId']),
  ...
  void _showEditCustomerDialog(Map<String, dynamic> customer) {
    final boxIdController = TextEditingController(text: customer['boxId'] ?? '');
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  // Box Number field (primary identifier for Sky)
                  _buildTextField(
                    controller: boxNumberController,
                    label: 'Box Number *',
                    icon: Icons.inventory_2,
                ...
                children: [
                  Expanded(
                    child: TextField(
                      ...
                      style: const TextStyle(color: Colors.white),
                      decoration: InputDecoration(
                        hintText: 'Search by name, box number, date...',
                        hintStyle: TextStyle(color: Colors.white.withValues(alpha: 0.5)),
                        prefixIcon: Icon(Icons.search, color: skyGradient[0]),
                  ...
                  Container(
                    decoration: BoxDecoration(
                      color: skyGradient[0].withValues(alpha: 0.15),
                      borderRadius: BorderRadius.circular(12),
                      border: Border.all(color: skyGradient[0]),
                    ),
                    child: IconButton(
                      icon: Icon(Icons.calendar_month, color: skyGradient[0]),
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 8),
                          _buildDetailRow('Box Number', widget.customer['boxNumber'] ?? 'N/A', copiable: true),
                          _buildDetailRow('Box ID', widget.customer['boxId'] ?? 'N/A'),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Scan a Box ID to add your first Sky customer',
            style: TextStyle(
              color: Colors.white.withValues(alpha: 0.6),
